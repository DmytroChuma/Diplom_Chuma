...
export default function Advertisements (props) {
    ...
    useEffect(() => {
        ...
        setShowPages(false);
        fetch(`/search?user=${props.id}&page=${page}`).then((res) => res.json()).then((data) => {
            setData(data.realty);
            setCount(data.count);
    ...
    const pagesHandler = (activePage) => {
        ...
        navigate(`/realtor/${props.id}/advertisements?user=${props.id}&page=${activePage}`);
    }

    document.title = 'Оголошення рієлтора';

    return (
        <div className="realtor-advertisement-container">
            {Array.isArray(data) && 
                <div>
                    <span className='span-count'>Кількість оголошень рієлтора: {count}</span>
                    <div className="separator"></div>
                </div>}
            {!Array.isArray(data) && data}
            {data.length === 0 && <NoResult text='Рієлтор не опубліковував оголошень' />}
