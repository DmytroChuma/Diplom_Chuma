...
export default function Realtors ({ id, user, dialog, socket}) {
    ...
    useEffect(() => {
        ...
        fetch(`https://diplomchuma-production.up.railway.app/get_realtors?agency=${id}`).then((res) => res.json()).then((data)=>{
            if (data.length > 0) {
                setData(data)
                setLoad(false)
                setNoRes(false)
        ...
        fetch(`https://diplomchuma-production.up.railway.app/get_agency_info?id=${id}`).then((res) => res.json()).then((data) => {
            setName(data.name);
        })
    }, [id, user])
    ...
    const removeRealtor = (realtorId, name) => {
        ...
        fetch('https://diplomchuma-production.up.railway.app/del_realtor', 
        {
            method: 'POST',
            headers: {
