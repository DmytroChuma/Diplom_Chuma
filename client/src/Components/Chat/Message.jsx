        ...
        <div className="message-chat">
             {(props.avatar || props.avatar === '') &&
                <div className="avatar-container-message">
                    <img className="user-avatar-message" src={'https://diplomchuma-production.up.railway.app/users/' + (props.avatar !== '' ? props.avatar : 'avatar.png')} alt=''/>
                </div>
            }
            <div className={"message " + props.class} onContextMenu={(e) => props.handleClick(e, props.class, props.text, (props.file || props.file === '') ? '' : props.file.path, props.id)}>
                ...
                <div className="message-chat-cont">
                    <span>{props.text !== '' ? parse(props.text) : props.file.type.split('/')[0] === 'image' ? <img className="chat-img-message" src={`https://diplomchuma-production.up.railway.app/files/${props.file.path}`} alt=''/> : <a href={`https://diplomchuma-production.up.railway.app/files/${props.file.path}`} download className="file-dummy-span">{props.file.path}</a>}</span>
